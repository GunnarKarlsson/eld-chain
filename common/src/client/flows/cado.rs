...
pub(crate) async fn get_cado(client: &ChainClient, path: String) {
    if path.contains("contract_info") || path.contains("contract_state") {
        ...
        match api.get_cado(path.clone()).await {
            Ok(response) => {
...
pub(crate) async fn list_cados(client: &ChainClient, search_string: String) {
    let api = AbciHttpApi::new(client.config.get_node_url().to_owned());
    match api.get_cado_paths(search_string.clone()).await {
        Ok(paths) => {
            info!(
        ...
        Err(e) => {
            error!("Error querying CADO paths: {}", e);
        }
    }
}
