...
impl AbciHttpApi {
    ...
    pub async fn get_capacity_validators(
        ...
        let response = self
            .client
            .abci_query(
                Some(crate::constants::abci_query::ACTIVE_CAPACITY_VALIDATORS.to_string()),
    ...
    /// Pinboard query (ABCI): request data is a UTF-8 path under [`crate::constants::cado::PATH_PREFIX_PINBOARD`].
    ///
    ...
    pub async fn pinboard_query(&self, path: String) -> Result<serde_json::Value, EldError> {
        let response = self
            .client
            .abci_query(
                Some(crate::constants::abci_query::PINBOARD.to_string()),
                path.as_bytes().to_vec(),
                None,
                false,
            )
...
pub fn decode_eld_tx_from_wire(tx_bytes: &[u8]) -> Result<crate::tx::Tx, EldError> {
...
pub fn decode_eld_tx_from_block_tx_bytes(tx_bytes: &[u8]) -> Result<crate::tx::Tx, EldError> {
