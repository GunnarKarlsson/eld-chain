...
//! a `0x` / `0X` prefix for boundary compatibility.
...
impl ContractId {
    ...
    ///
    /// Accepts inputs with or without `0x` / `0X` prefix and normalizes to canonical output via
    /// [`std::fmt::Display`].
    ...
    fn parse_hex(s: &str) -> Result<Self, EldError> {
        let bytes = decode_fixed_hex::<{ Self::LEN }>(s, "contract ID")?;
        Ok(Self { bytes })
    }
...
mod tests {
    ...
    fn parse_accepts_with_or_without_prefix() {
        ...
        let upper_prefix =
            ContractId::parse(Some(&format!("0X{}", &SAMPLE[2..]))).expect("valid 0X prefix");
