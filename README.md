...
| **bincode** | CADO payload bytes (`Account`, staking accounts, `EpochRecord`, `NamespaceRecord`, contract info/state, CADO envelope). The node also uses bincode for GossipSub `SyncMsg` and persisted pinboard metadata. |
