...
impl StorageConfig {
    ...
    /// Validate all fields in the storage config
    pub fn validate(&self) -> Result<(), eld_common::error::EldError> {
        // Validate storage_host
        validate_ip_or_hostname(&self.storage_host)?;

        // Validate storage_port
        validate_port(&self.storage_port)?;

        Ok(())
    }

    pub fn ip_address(&self) -> String {
        format!("{}:{}", self.storage_host, self.storage_port)
    }
    ...
    /// Load storage configuration and ensure the backing database directory is secure.
    pub fn load_for_node(
        storage_config_path: &str,
        db_path: &str,
    ) -> Result<(Self, String), EldError> {
        // Ensure DB directory exists and has secure permissions
        Self::validate_db_directory_permissions(db_path)?;

        // Load configuration from file (legacy loader) and validate it
        let cfg = StorageConfig::from_file(storage_config_path);
        cfg.validate()?;

        Ok((cfg, db_path.to_string()))
    }

    /// Validate and secure directory permissions for database
    fn validate_db_directory_permissions(db_path: &str) -> Result<(), EldError> {
        use std::fs;
        use std::os::unix::fs::PermissionsExt;
        ...
        if !path.exists() {
            fs::create_dir_all(&path).map_err(|e| EldError::FileSystemError {
            ...
            tracing::info!(
                ...
                eld_client::logging::SanitizedLog::as_path(db_path)
            );
        }

        // Get directory metadata
        let metadata = fs::metadata(&path).map_err(|e| EldError::FileSystemError {
            operation: "get_directory_metadata".to_string(),
            path: db_path.to_string(),
            details: format!("Failed to get directory metadata: {e}"),
        })?;

        // Check directory permissions (should be read/write/execute for owner only)
        let permissions = metadata.permissions();
        let mode = permissions.mode();
        ...
        if mode & 0o777 != 0o700 {
            tracing::warn!(
                "Database directory has insecure permissions: {:o}. Recommended: 700 (owner read/write/execute only)",
                mode & 0o777
            );
            ...
            #[cfg(unix)]
            {
                let new_permissions = fs::Permissions::from_mode(0o700);
                fs::set_permissions(&path, new_permissions).map_err(|e| {
                ...
                tracing::info!("Set secure permissions on database directory");
            }
        }
        ...
        #[cfg(unix)]
        {
            ...
            let current_uid = unsafe { libc::getuid() };
            if metadata.uid() != current_uid {
                tracing::warn!("Database directory is not owned by current user");
            }
        }
...
mod tests {
    ...
    fn test_storage_config_validation_valid() {
        ...
        let config = StorageConfig::from_file(
            temp_file
                .path()
                .to_str()
                .expect("Unable to get temp file path"),
        );
        assert_eq!(config.storage_host, "0.0.0.0");
        assert_eq!(config.storage_port, "8000");
    }

    #[test]
    fn test_storage_config_validation_with_hostname() {
        ...
        assert_eq!(config.storage_host, "localhost");
        assert_eq!(config.storage_port, "8000");
    }
    ...
    fn test_storage_config_validate_method() {
        ...
        let mut config = StorageConfig {
            storage_host: "0.0.0.0".to_string(),
            storage_port: "8000".to_string(),
        };
        ...
        // Test invalid host
        config.storage_host = "".to_string();
        assert!(config.validate().is_err());

        // Test invalid port
        config.storage_host = "0.0.0.0".to_string();
        config.storage_port = "0".to_string();
        assert!(config.validate().is_err());
    }

    #[test]
    fn test_storage_config_ip_address() {
        ...
        assert_eq!(ip_address, "0.0.0.0:8000");
    }
}
