...
Tx {
    ...
    payload: struct PostMessageTx { // this is the inner payload for a Tx
        // Fields provider by user
        // "Message commitment" fields, not the struct
    }
}
```
...
1. **Client preparation**  
   ...
   - Sign `MessageCommitment` (excluding `message_id` from signature input)
...
2. **REST API call** – `POST /pinboard/submit` (to validator node)  
   ```json
   {
     ...
     "payload":          "base64-encoded-full-message-payload"
   }
   ```
